...
//
// WHEN API SPEC IS READY:
//   Replace the fetchCorporateFamily() function body.
//   The rest of the component (tree render, selection, FAB) is API-agnostic.

const CorporateTree = (() => {

  // ── Storage helpers (same as credentials component) ────────────────────────
  function storageGet(keys) {
    return new Promise(resolve => chrome.storage.local.get(keys, resolve));
  }

  // ── API ────────────────────────────────────────────────────────────────────
  // TODO: replace with real Intapp Party API call once spec is available.
  // Expected return shape: array of flat nodes that will be built into a tree.
  //   { id: string, name: string, parentId: string|null, metadata: object }
  async function fetchCorporateFamily(query, token, appHost) {
    ...
    // Real call will look roughly like:
    // const res = await fetch(
    //   `https://${appHost}/api/party/v1/parties/${encodeURIComponent(query)}/corporate-family`,
    //   { headers: { Authorization: `Bearer ${token}` } }
    // );
    // if (!res.ok) throw new Error(`API ${res.status}`);
    // const data = await res.json();
    // return normaliseNodes(data); // map API shape → { id, name, parentId, metadata }
  }

  // Mock data mirrors the structure visible in the screenshot
  const MOCK_FAMILY = [
    { id: '196337864', name: 'Meta Platforms, Inc.',            parentId: null },
    { id: '119440644', name: 'Blocke LLC',                      parentId: '196337864' },
    { id: '600574240', name: 'DOGE HOLDINGS LTD',               parentId: '196337864' },
    { id: '533787722', name: 'FACEBOOK ISRAEL LTD',             parentId: '600574240' },
    { id: '119486768', name: 'Dotier LLC',                      parentId: '196337864' },
    { id: '203595145', name: 'Facebook Canada Ltd',             parentId: '196337864' },
    { id: '119577966', name: 'Facebook Circularity, LLC',       parentId: '196337864' },
    { id: '080762681', name: 'Facebook Global Holdings I, LLC', parentId: '196337864' },
    { id: '965522118', name: 'Facebook Global Holdings II, LLC',parentId: '196337864' },
    { id: '472569645', name: 'CADENA NETWORKS SL.',             parentId: '965522118' },
    { id: '070090142', name: 'FACEBOOK (THAILAND) COMPANY LTD',parentId: '965522118' },
  ];

  // ── Tree builder ────────────────────────────────────────────────────────────
  // Takes flat node array, returns nested tree.
  ...
  // ── State ──────────────────────────────────────────────────────────────────
  const state = {
    selected: new Set(),  // Set of node IDs
    collapsed: new Set(), // Set of collapsed node IDs
    nodes: [],            // flat node array
    onSelect: null,
    ...
    return `
      <div class="ct-node" data-id="${node.id}">
        <div class="ct-node-row ${isSelected ? 'selected' : ''}" data-row="${node.id}">
          ...
          <span class="ct-id">${esc(node.id)}</span>
        </div>
        <div class="ct-children ${isCollapsed ? 'collapsed' : ''}" data-children="${node.id}">
  ...
  function renderTree(treeEl, roots) {
    ...
    treeEl.querySelectorAll('[data-toggle]').forEach(btn => {
      btn.addEventListener('click', e => {
        ...
        renderTree(treeEl, roots);
        updateSelBar();
      });
    });

    // Toggle selection (checkbox or row click)
    treeEl.querySelectorAll('[data-row]').forEach(row => {
      row.addEventListener('click', e => {
        if (e.target.dataset.toggle) return; // handled above
  ...
  function mount(selector = '#tree-root', opts = {}) {
        ...
        <div class="ct-header">
          ...
          <div class="ct-search-row">
            <input class="ct-input" id="ct-query" type="text"
              placeholder="Party name or ID"
              autocomplete="off" spellcheck="false" />
            <button class="ct-load-btn" id="ct-load">Load</button>
          </div>
        </div>
        <div class="ct-status" id="ct-status">Enter a party name or ID to load its corporate family.</div>
        <div class="ct-tree" id="ct-tree"></div>
        <div class="ct-sel-bar" id="ct-sel-bar">
    ...
    async function load() {
      ...
      try {
        ...
        statusEl.textContent = `${state.nodes.length} entities loaded.`;
        const roots = buildTree(state.nodes);
        renderTree(treeEl, roots);
      } catch (err) {
    ...
    actionBtn.addEventListener('click', () => {
      ...
      const entities = [...state.selected].map(id => nodeMap[id]).filter(Boolean);
...
document.addEventListener('DOMContentLoaded', () => {
  if (document.getElementById('tree-root')) {
    CorporateTree.mount('#tree-root', {
      onSelect: ({ entities }) => {
        // Default: log selected entities.
        // Replace with postMessage / chrome.runtime.sendMessage when wired to Celeste.
