// ─── credentials.js ──────────────────────────────────────────────────────────
// Drop-in credentials manager for Chrome extensions.
// Version: 1.1.4
//
// CHANGELOG:
...
//   1.0.5 — Combined CapIQ + Xpressapi into single PR_EXT_credentials_worker.js (/xpx/* routes)
  ...
  function wireButtons() {
   ...
   <soapenv:Body>
    ...
    document.getElementById('creds-test-api').addEventListener('click', async () => {
      ...
      try {
        const res = await chrome.runtime.sendMessage({ type: 'FETCH_CORPORATE_FAMILY', partyId });
        if (res.ok) {
          const trees = res.data?.corporateTrees;
          out.textContent = trees
            ? `✅ ${trees.length} tree(s): ${trees.map(t => `${t.name} (${t.providerType})`).join(', ')}`
            : `✅ OK but no corporateTrees. Keys: ${Object.keys(res.data).join(', ')}`;
        } else {
