...
let configuredProviders = []; // populated on load
...
async function renderHistory(filter) {
  ...
  historyDropdown.querySelectorAll('.hist-del').forEach(el => {
    el.addEventListener('click', (e) => {
      e.stopPropagation();
      removeFromHistory(el.dataset.del);
    });
  });
}
...
document.addEventListener('click', (e) => {
  if (!e.target.closest('.name-field')) historyDropdown.classList.remove('open');
});
...
function updateStatusDots(s) {
  const dot = (id, val, cls) => {
    const el = document.getElementById(id);
    el.textContent = val ? '✓ Set' : 'Not set';
    el.className = 'settings-status ' + (val ? cls : '');
  };
...
document.querySelectorAll('[data-clear]').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    const keys = p === 'moodys' ? ['moodys_username','moodys_password','moodys_base_url']
               : p === 'dnb'    ? ['dnb_basic_token']
...
function flashMsg(p, msg, cls) {
  ...
  clearTimeout(el._t);
  el._t = setTimeout(() => el.className = 'save-flash', 2500);
  // Refresh status dots
...
function buildProviderSelector() {
  ...
  const noCreds = document.getElementById('noCredsMsg');
  ...
  // Build pills: All + each configured provider
  const labels = { moodys: "Moody's", dnb: 'D&B', sp: 'S&P' };
  let html = '';
  if (configuredProviders.length > 1) {
    html += `<div class="provider-opt sel-all" data-v="all">All</div>`;
  }
  configuredProviders.forEach(p => {
    html += `<div class="provider-opt" data-v="${p}">${labels[p]}</div>`;
  });
  sel.innerHTML = html;

  // If only one provider, select it and hide the row
  if (configuredProviders.length === 1) {
    selectedProvider = configuredProviders[0];
    row.style.display = 'none';
  } else {
    selectedProvider = 'all';
    sel.querySelector('[data-v="all"]').classList.add('sel-all');
  }
...
function buildTabs(providers) {
  ...
  providers.forEach(p => {
    const [label, cls] = cfg[p];
    html += `<div class="tab ${cls}" data-tab="${p}">${label} <span class="tab-count" id="${p}Count">—</span></div>`;
  });
  tabBar.innerHTML = html;

  // If only one provider, show its panel directly
  if (!multi) {
    document.getElementById('panelMatched').classList.remove('active');
    document.getElementById(`panel${cap(providers[0])}`).classList.add('active');
  } else {
    document.getElementById('panelMatched').classList.add('active');
  }
  ...
  tabBar.addEventListener('click', e => {
    ...
    const panelId = name === 'matched' ? 'panelMatched' : `panel${cap(name)}`;
...
async function doSearch() {
  ...
  ['moodysBody','dnbBody','spBody','matchedBody'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.innerHTML = `<div class="panel-loading"><span class="spinner"></span>Loading…</div>`;
  });
  ...
  chrome.storage.local.get(['moodys_username','moodys_password','moodys_base_url','dnb_basic_token','sp_username','sp_password'], async s => {
    ...
    await Promise.all(Object.entries(tasks).map(async ([p, task]) => {
      if (!task) return;
      try { results[p] = { ok: true, data: await task }; }
      catch(e) { results[p] = { ok: false, err: e.message }; }
    }));
...
async function runMoodys(s, name) {
  ...
  while ((m = re.exec(matchXml)) !== null) {
    const b = m[1];
    out.push({
      id: xmlTag(b,'BvDID'), name: xmlTag(b,'Name'),
      address: xmlTag(b,'Address'), postCode: xmlTag(b,'PostCode'),
      city: xmlTag(b,'City'), country: xmlTag(b,'Country'),
      website: xmlTag(b,'EMailOrWebsite'),
      ticker: xmlTag(b,'Ticker'), isin: xmlTag(b,'ISIN'), lei: xmlTag(b,'LEI'),
    });
  }
  return out;
}
...
// S&P — calls are routed through background.js to avoid CORS restrictions
...
async function getSpToken(username, password) {
  ...
  const res = await fetch(SP_PROXY_URL + '/token', {
    method: 'POST',
...
async function runSp(s, name) {
  if (!s.sp_username || !s.sp_password) {
    throw new Error('S&P credentials not set — open Settings and save your username and password.');
  }
  ...
  const searchRes = await fetch(SP_PROXY_URL + '/search', {
    method: 'POST',
  ...
  if (!searchRes.ok) {
    ...
    throw new Error('S&P search failed (HTTP ' + searchRes.status + '): ' + searchText.slice(0, 200));
  }
  ...
  for (let i = 0; i < nameRows.length; i++) {
    ...
    out.push({ id: companyId, name: companyName, location: '', website: '', ticker: '', isin: '', lei: '' });
    ids.push(companyId);
  }

  if (!ids.length) return out;

  // Enrich with GDSP — batch all IDs per mnemonic
  try {
    ...
    const enrichRes = await fetch(SP_PROXY_URL + '/search', {
      method: 'POST',
      headers: { 'Authorization': 'Bearer ' + accessToken, 'Content-Type': 'application/json' },
      body: enrichBody,
    });
    ...
    if (enrichRes.ok) {
      const enrichData = JSON.parse(await enrichRes.text());
      const er = enrichData.GDSSDKResponse || [];
      const n = ids.length;
      // Responses come back in order: tickers[0..n-1], websites[n..2n-1], addresses[2n..3n-1], countries[3n..4n-1]
      const getVal = (block) => block && block.Rows && block.Rows[0] && block.Rows[0].Row && block.Rows[0].Row[0] || '';
      for (let i = 0; i < out.length; i++) {
        out[i].ticker   = getVal(er[i]);
        out[i].website  = getVal(er[n + i]);
        out[i].location = [getVal(er[2*n + i]), getVal(er[3*n + i])].filter(Boolean).join(', ');
      }
    }
  } catch (e) {
    // Enrichment failed — return basic results
    console.warn('S&P enrichment failed:', e.message);
  }
...
function renderProviderPage(p) {
  ...
  slice.forEach(c => {
    ...
    // Build tooltip rows for any extra fields that have values
    // Split ticker on ':' → e.g. "NasdaqGS:SBUX" → market + ticker
    let tickerVal = c.ticker || '';
    let marketVal = '';
    if (tickerVal.includes(':')) {
      const parts = tickerVal.split(':');
      marketVal = parts[0].trim();
      tickerVal = parts[1].trim();
    }

    const tooltipFields = [
      { key: 'Market',  val: marketVal },
      { key: 'Ticker',  val: tickerVal },
      { key: 'Website', val: c.website },
      { key: 'ISIN',    val: c.isin },
      { key: 'LEI',     val: c.lei },
      { key: 'Location',val: loc },
    ].filter(f => f.val);

    const tooltipHtml = tooltipFields.length ? `
      <div class="card-tooltip">
        ${tooltipFields.map(f => `<div class="card-tooltip-row"><span class="card-tooltip-key">${f.key}</span><span class="card-tooltip-val">${escHtml(f.val)}</span></div>`).join('')}
      </div>` : '';

    // Build inline meta items
    const metaItems = [
      { key: 'market',   val: marketVal },
      { key: 'ticker',   val: tickerVal },
      { key: 'website',  val: c.website },
    ...
    const metaHtml = metaItems.length ? `<div class="card-meta">${
      metaItems.map(f => `<span class="card-meta-item"><span class="card-meta-key">${f.key}</span><span class="card-meta-val">${escHtml(f.val)}</span></span>`).join('')
    }</div>` : '';
    ...
    card.innerHTML = `
      <div class="card-id-row"><span class="card-id ${idCls}">${escHtml(c.id||'—')}</span><span class="copy-hint">copy ⧉</span></div>
  ...
  renderPagination(pagEl, page, total, np => {
    if (p === 'moodys') moodysPage = np; else if (p === 'dnb') dnbPage = np; else spPage = np;
    renderProviderPage(p); body.parentElement.scrollTop = 0;
  }, p);
}
...
function buildMatches(moodys, dnb, sp, squishy) {
  ...
  for (const idKey of ['ticker','isin','lei']) {
    ...
    allProviders.forEach(({ key, arr }) => {
      ...
      arr.forEach((r,i) => {
        ...
        // Strip exchange prefix from ticker e.g. "NASDAQGS:SBUX" → "SBUX"
  ...
  if (squishy) {
    for (const [label, extractor] of [
      ...
      ['Address', r => normaliseAddress([r.address||'', r.city||''].join(' ') || (r.location||'').split(',').slice(0,2).join(' '))],
    ]) {
      const indexes = {};
      allProviders.forEach(({ key, arr }) => {
        indexes[key] = {};
...
function renderMatched() {
  const active = [
    moodysCandidates.length ? 'moodys' : null,
    dnbCandidates.length    ? 'dnb'    : null,
    spCandidates.length     ? 'sp'     : null,
  ].filter(Boolean);

  const { matches, unmoodys, undnb, unsp } = buildMatches(moodysCandidates, dnbCandidates, spCandidates, squishyEnabled);
  const el = document.getElementById('matchedCount');
  if (el) el.textContent = matches.length;

  const body = document.getElementById('matchedBody');
  if (!matches.length) {
    body.innerHTML = '<div class="no-results">No cross-provider matches found.<br>Try Squishy Match, or browse individual tabs.</div>';
    return;
  }

  const colHtml = (p, result) => {
    const dotCls  = `dot-${p}`;
    const idCls   = `col-id-${p}`;
    const label   = p === 'moodys' ? "Moody's" : p === 'dnb' ? 'D&B' : 'S&P';
    if (!result) return `<div class="match-col"><div class="col-provider"><div class="col-dot ${dotCls}"></div><span class="col-provider-name">${label}</span></div><div class="col-empty">No match</div></div>`;
    const id  = result.id || '—';
    ...
    return `<div class="match-col">
      <div class="col-provider"><div class="col-dot ${dotCls}"></div><span class="col-provider-name">${label}</span></div>
      <div class="col-id-val ${idCls}" data-copy="${escHtml(id)}" style="cursor:pointer">${escHtml(id)} <span style="font-size:9px;opacity:.4">⧉</span></div>
      <div class="col-name">${escHtml(result.name||'—')}</div>
      ${loc ? `<div class="col-sub">${escHtml(loc)}</div>` : ''}
    </div>`;
  };

  body.innerHTML = '';
  // Set grid columns to match number of active providers
  ...
  matches.forEach(m => {
    ...
    row.innerHTML = `
      <div class="match-key-bar">
        <span class="match-key-label">${escHtml(m.key)}</span>
        <span class="match-key-val">${escHtml(m.val)}</span>
        <span class="match-key-badge ${m.type === 'exact' ? 'badge-exact' : 'badge-squishy'}">${m.type === 'exact' ? 'Exact' : 'Squishy'}</span>
      </div>
      <div class="match-cols" style="--match-cols:${colCount}">
        ${active.map(p => colHtml(p, m[p])).join('')}
      </div>`;
    row.querySelectorAll('[data-copy]').forEach(el => {
      el.addEventListener('click', () => navigator.clipboard.writeText(el.dataset.copy).then(() => showToast('Copied: ' + el.dataset.copy)));
    });
