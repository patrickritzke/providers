  ...
  logListEl.innerHTML = allLogs.map(entry => {
    ...
    return `<div class="log-entry${active}" data-id="${entry.id}">
      <div class="entry-top">
        ...
        <div class="entry-action"><span class="action-name">${escHtml(actionShort)}</span></div>
      </div>
      <div class="entry-meta">
        <span class="entry-time">${dateStr} ${timeStr}</span>
        ${entry.durationMs !== null ? `<span class="entry-duration">${entry.durationMs}ms</span>` : ''}
...
function renderDetail(entry) {
  ...
  const statusPill = entry.pending
    ? `<span class="pill">pending</span>`
    : isOk
      ? `<span class="pill ok">HTTP ${entry.status} OK</span>`
      : `<span class="pill err">${entry.error ? 'Error: ' + escHtml(entry.error) : 'HTTP ' + entry.status}</span>`;
  ...
  detailEl.innerHTML = `
    <div class="detail-header">
      ...
      <div class="detail-pills">
        ${statusPill}
        ${entry.durationMs !== null ? `<span class="pill">${entry.durationMs}ms</span>` : ''}
        <span class="pill">${escHtml(ts)}</span>
        <span class="pill" title="${escHtml(entry.url)}">${escHtml(trimUrl(entry.url))}</span>
      </div>
    </div>
    ...
    <div class="section">
      <div class="section-label">
        Action
        <button class="copy-btn" data-copy="${escHtml(entry.action)}">copy</button>
      </div>
      <pre>${escHtml(entry.action)}</pre>
    </div>

    <div class="section">
      <div class="section-label">
        Request Body
        <button class="copy-btn" data-copy-pre="req">copy</button>
      </div>
      <pre id="pre-req">${highlightXml(entry.requestBody || '')}</pre>
    </div>

    <div class="section">
      <div class="section-label">
        Response Body
        <button class="copy-btn" data-copy-pre="res">copy</button>
      </div>
      <pre id="pre-res">${entry.responseBody ? highlightXml(entry.responseBody) : entry.pending ? '(pending…)' : '(empty response body)'}</pre>
    </div>

    ${entry.error ? `
    <div class="section">
      <div class="section-label">Error</div>
      <pre style="color:var(--danger)">${escHtml(entry.error)}</pre>
    </div>` : ''}
  `;
  ...
  detailEl.querySelectorAll('.copy-btn[data-copy-pre]').forEach(btn => {
    btn.addEventListener('click', () => {
      const pre = document.getElementById('pre-' + btn.dataset.copyPre);
...
function highlightXml(xml) {
  if (!xml) return '';
  // Escape first, then apply highlighting spans
  return escHtml(xml)
    // XML comments
    .replace(/(&lt;!--[\s\S]*?--&gt;)/g, '<span class="xml-comment">$1</span>')
    // Tags: capture <tagname attrs> or </tagname> or <tagname/>
    .replace(/(&lt;\/?)([\w:.-]+)([^&]*?)(\/?&gt;)/g, (_, open, name, attrs, close) => {
      ...
      return `<span class="xml-tag">${open}${name}</span>${highlightedAttrs}<span class="xml-tag">${close}</span>`;
    });
}
