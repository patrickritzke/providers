...
const Logger = {
  ...
  async trackedSoapCall(url, action, body) {
    ...
    const requestEntry = { id, timestamp, action, url, requestBody: body, status: null, responseBody: null, durationMs: null, error: null, pending: true };
    ...
    try {
      ...
      await Logger._updateEntry(id, { status: response.status, responseBody, durationMs, pending: false });
      if (!response.ok) throw new Error(`HTTP ${response.status} ${response.statusText}`);
      return responseBody;
    } catch (err) {
      const durationMs = Math.round(performance.now() - start);
      await Logger._updateEntry(id, { status: null, responseBody: null, durationMs, error: err.message, pending: false });
      throw err;
    }
  },
